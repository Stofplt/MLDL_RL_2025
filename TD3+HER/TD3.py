...
class TD3(object):
	def __init__(
		...
		use_rank=False,
		use_hindsight=False,
		actor_lr=3e-4,  # NEW: allow actor_lr
		critic_lr=3e-4  # NEW: allow critic_lr
	):
	...
	def train(self, replay_buffer, batch_size=100, stage=None):
		self.total_it += 1

		# Sample replay buffer 
		if hasattr(replay_buffer, 'her_k'):
			# HERReplayBuffer
			state, action, next_state, reward, not_done = replay_buffer.sample(batch_size, stage=stage)
			# Move to device
